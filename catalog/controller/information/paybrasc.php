 ...
 **/
class ControllerInformationPaybrasC extends Controller 
{
   private $error = array();
      
     public function index() {
      $this->language->load('information/paybrasc'); 
	   ...
	   else if($tipo==3)
	   {
		...
		else if($tipo==5)
		{
		   ...
		   $this->data['conteudo_centro'] = 'Seu pedido foi enviado com sucesso. Contudo, é necessário que seja efetuado o pagamento do boleto bancário para que ele possa ser processado. Para imprimir seu boleto, clique no botão abaixo:<BR><BR><a href="'.$this->config->get('config_url').'.html" target="_blank"><img src="image/botao_imprimir_boleto.png" /></a><BR>';
      ...
      if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/paybrasc.tpl')) 
	  { 
         $this->template = $this->config->get('config_template') . '/template/information/paybrasc.tpl';
      } else {
         $this->template = 'default/template/information/paybrasc.tpl'; 
      }

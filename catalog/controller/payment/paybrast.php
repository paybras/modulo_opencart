  ...
  public function get_ip()
{
  ...
  protected function index() 
  {
	...
	$this->data['desconto'] = $this->config->get('paybrast_desconto');
    ...
    $this->id = 'payment';
    if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paybrast.tpl')) {
	  $this->template = $this->config->get('config_template') . '/template/payment/paybrast.tpl';
	} else {
	  $this->template = 'default/template/payment/paybrast.tpl';
	}	
	$this->render();
  }
...
if(!$pedido->row['shipping_address_2']) $pedido->row['shipping_address_2']='N/D';
	...
	$dados['recebedor_api_token'] = $this->config->get('paybrast_token');
	$dados['recebedor_email'] = $this->config->get('paybrast_email_conta');
	...
	$dados['pedido_url_redirecionamento'] = $this->config->get('config_url').'/index.php?route=information/paybrast&tipo=4';
	...
	$dados['pagador_cpf'] = $request->post['cpf'];
	$dados['pagador_rg'] = '';
	...
	if($this->config->get('paybrast_modo')==2)
	 ...
	 if($resultado->status_codigo==1 && $resultado->sucesso==1)
	{
		#Atualiza o status da transacao na tabela dos pedidos	
            $this->db->query("UPDATE ".DB_PREFIX."order SET id_transacao_paybras ='" .$transacao . "', url_paybras='".$url_pagamento."###' WHERE order_id=".$pedido->row['order_id']);
			
		$url_boleto=$this->config->get('config_url').'pagar.php?id='.md5($transacao);
		$botao_boleto='<a href="'.$this->config->get('config_url').'pagar.php?id='.md5($transacao).'" target="_blank"><img src="'.$this->config->get('config_url').'image/paybras/'.str_replace('tef_','',$_POST['banco']).'_concluir.png" align="left" /></a>';
		#Aguardando pagamento / Pagamento em Análise
			$output = "<script>window.location.href = '".$url_pagamento."';</script>"; 
            $this->model_checkout_order->confirm($order_id, $this->config->get('paybrast_aguardando_pagamento'), $comment = '<P>Caso ainda não tenha efetuado o pagamento, vocÊ pode realizá-lo utilizando o seguinte link: </P>'.$botao_boleto.'<BR><BR><P>Link direto:'.$url_boleto."</P>", $notfy = true);
	...
	else
	{
		#Ocorreu um erro no pagamento
            ...
            $output = "<script>window.location = 'index.php?route=information/paybrast&tipo=4';</script>";
            $this->model_checkout_order->confirm($order_id, $this->config->get('paybrast_pagamento_erro'), $comment = 'TEF', $notify = false);
	}
